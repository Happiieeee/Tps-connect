...
router.get('/students', verifyTeacherPasscode, async (req, res) => {
  try {
    const query = `
      SELECT s.student_id, s.name AS student_name, s.dob, s.admission_date, s.emergency_contact, s.is_active,
      ...
      ORDER BY b.name, c.class_name, s.name
    `;
...
router.put('/student/:id', verifyTeacherPasscode, async (req, res) => {
  ...
  try {
    ...
    // 3. Link Student and Parent (clean up old parents link first for consistency)
    await client.query(
      'DELETE FROM student_parents WHERE student_id = $1',
      [studentId]
    );
