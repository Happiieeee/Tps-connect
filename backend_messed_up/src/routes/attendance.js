...
router.post('/mark', verifyToken, requireRole(['superadmin', 'branchadmin', 'teacher']), async (req, res) => {
  try {
    ...
    // Lock attendance after 4pm for teachers
    const lockHour = 16;
    if (markDate.toDateString() === now.toDateString() && now.getHours() >= lockHour && role === 'teacher') {
      return res.status(403).json({ error: 'Attendance is locked after 4:00 PM' });
    }
    ...
    const studentNames = await pool.query(
      `SELECT student_id, name FROM students WHERE student_id = ANY($1)`,
      [records.map(r => r.student_id)]
    );
    const nameMap = {};
    studentNames.rows.forEach(s => { nameMap[s.student_id] = s.name; });

    const detailedStudents = records.map(r => ({
      name: nameMap[r.student_id] || r.student_id,
      status: r.status,
    }));
    ...
    await pool.query(
      ...
      [user_id, branch_id, JSON.stringify({
        class_id: class_id,
        date: date,
