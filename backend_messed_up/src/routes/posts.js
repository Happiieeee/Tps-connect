...
router.post('/', verifyToken, requireRole(['superadmin', 'branchadmin', 'teacher']), async (req, res) => {
  try {
    ...
    const {
      ...
      class_id, file_urls, target_branch_id,
      scheduled_at  // NEW — ISO string or null
    } = req.body;
    ...
    const isPublished  = !isScheduled; // scheduled posts start unpublished

    const result = await pool.query(
      `INSERT INTO posts
         (branch_id, class_id, posted_by, category, title, content,
          file_urls, is_scheduled, scheduled_at, is_published)
    ...
    await pool.query(
      `INSERT INTO teacher_logs (user_id, branch_id, action, meta, timestamp)
       VALUES ($1,$2,$3,$4,NOW())`,
      [user_id, assignedBranch, isScheduled ? 'schedule_post' : 'create_post',
       JSON.stringify({ post_id: post.post_id, category, title, scheduled_at })]
    );

    // Only notify immediately if NOT scheduled
    if (!isScheduled) {
      ...
      await notifyBranch(
        assignedBranch, class_id || null,
        categoryLabels[category] || 'New Post', title
      );
    }
    ...
    res.status(201).json({
      ...post,
      message: isScheduled
        ? `Post scheduled for ${new Date(scheduled_at).toLocaleString()}`
...
router.put('/:id', verifyToken, requireRole(['superadmin', 'branchadmin', 'teacher']), async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `UPDATE posts SET title=$1, content=$2, file_urls=$3, updated_at=NOW()
       WHERE post_id=$4 RETURNING *`,
      [title, content, file_urls || post.file_urls, req.params.id]
    );
