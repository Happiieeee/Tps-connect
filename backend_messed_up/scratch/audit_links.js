  ...
  try {
    ...
    if (parents.rows.length > 0) {
      ...
      const students = await pool.query(
        `SELECT s.student_id, s.name as student_name, sp.parent_id, u.name as parent_name, u.phone
         FROM students s
    ...
    const posts = await pool.query(
      "SELECT post_id, title, author_id, branch_id, class_id, created_at FROM posts ORDER BY created_at DESC LIMIT 5"
    );
