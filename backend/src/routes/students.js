...
// GET all students in a branch (optionally filtered by class_id)
router.get('/', verifyToken, requireRole(['superadmin', 'branchadmin', 'teacher']), async (req, res) => {
  try {
    ...
    let queryStr = `SELECT s.student_id, s.name, s.dob, s.photo_url, s.class_id,
...
router.put('/:id', verifyToken, requireRole(['superadmin', 'branchadmin']), async (req, res) => {
  try {
    ...
    const existing = student.rows[0];
    const result = await pool.query(
      `UPDATE students 
       SET name=COALESCE($1, name), dob=COALESCE($2, dob), photo_url=COALESCE($3, photo_url),
           class_id=COALESCE($4, class_id), emergency_contact=COALESCE($5, emergency_contact),
           medical_notes=COALESCE($6, medical_notes), updated_at=NOW()
       WHERE student_id=$7
       RETURNING *`,
      [name ?? existing.name, dob ?? existing.dob, photo_url ?? existing.photo_url,
       class_id ?? existing.class_id, emergency_contact ?? existing.emergency_contact,
       medical_notes ?? existing.medical_notes, req.params.id]
    );
